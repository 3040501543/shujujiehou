 ...
 * 客户端启动类
 */
public class DemoClientMonitorEvent {

    private final String host;
    private final int port;

    public DemoClientMonitorEvent(String host, int port) {
        this.host = host;
        this.port = port;
    }
            ...
            bs.group(group)
                    ...
                    .handler(new ChannelInitializer<SocketChannel>() {//进行通道初始化配置
                        ...
                        protected void initChannel(SocketChannel socketChannel) throws Exception {
                            socketChannel.pipeline().addLast(new HandlerClientMonitorEvent());//添加自定义的handler
                        }
                    });
            //连接远程节点；等待连接完成
    ...
    public static void main(String[] args) throws Exception {
        new DemoClientMonitorEvent("127.0.0.1", 18080).run();
    }
