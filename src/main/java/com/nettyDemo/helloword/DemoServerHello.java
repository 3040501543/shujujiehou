...
public class DemoServerHello {

    private int port;

    public DemoServerHello(int port) {
        this.port = port;
    }
            ...
            bs.group(group)
                    ...
                    .childHandler(new ChannelInitializer<SocketChannel>() {//进行通道初始化配置
                        ...
                        protected void initChannel(SocketChannel socketChannel) throws Exception {
                            socketChannel.pipeline().addLast(new HandlerServerHello());//添加自定义的handler
                        }
                    });
            //连接远程节点；等待连接完成
